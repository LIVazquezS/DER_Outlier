...
class PhysNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.nuscale = nn.Parameter(torch.ones(95, device=self.device, dtype=dtype))
        ...
        self.output_block_evid = nn.ModuleList([OutputBlock(
            F, num_residual_output, n_output=5, activation_fn=self.activation_fn, rate=self.rate,device=self.device)
            for _ in range(self.num_blocks)])

        self.output_block_md_evid = nn.ModuleList([OutputBlock(
            F, num_residual_output, n_output=6, activation_fn=self.activation_fn, rate=self.rate,device=self.device)
            for _ in range(self.num_blocks)])
    ...
    def multidimensional_evidential_layer(self,Ea,Qa,l00,l10,l11,nu):
        """
        Multidimensional evidential layer for 2 degrees of freedom (E and Q).
        Taken from Meinert and Lavin, 2022 (https://arxiv.org/abs/2104.06135)

        """
        min_val = 1e-6
        mu_e = Ea
        mu_q = Qa
        L00 = torch.nn.Softplus()(l00) + min_val
        L10 = l10 + min_val
        L11 = torch.nn.Softplus()(l11) + min_val
        nu = 10.0*(torch.tanh(nu)+1.0)/2.0 + 3.0

        v = torch.stack((mu_e,mu_q,L00,L10,L11,nu),dim=-1)
        return v
    ...
    @torch.jit.export
    def multidimensional_evidential_atomic_properties(self, Z, R, idx_i, idx_j, offsets=None, sr_idx_i=None,
                                                      sr_idx_j=None,
                                                      sr_offsets=None):
        ''' Calculate evidential atomic properties using the multidimensional model '''
        ...
        Ea = 0  # atomic energy
        Qa = 0  # atomic charge
        l00, l10, l11, nu = 0, 0, 0, 0
        nhloss = 0  # non-hierarchicality loss
        for i in range(self.num_blocks):
            x = self.interaction_block[i](x, rbf, sr_idx_i, sr_idx_j)
            out = self.output_block_md_evid[i](x)
            Ea = Ea + out[:, 0]
            Qa = Qa + out[:, 1]
            l00 = l00 + out[:, 2]
            l10 = l10 + out[:, 3]
        ...
        l00 = self.lscale[Z.type(torch.int64)] * l00
        l10 = self.ascale[Z.type(torch.int64)] * l10
        l11 = self.bscale[Z.type(torch.int64)] * l11
        nu = self.nuscale[Z.type(torch.int64)] * nu

        return Ea, Qa, l00, l10, l11, nu, Dij_lr, nhloss
...
# Here you compute only the energy as the sum and add the dispersion corrections
    @torch.jit.export
    ...
    @torch.jit.export
    def energy_md_evidential_from_atomic_properties(
            self, Ea,Qa, l00,l10,l11,nu, Dij, Z, idx_i, idx_j, batch_seg=None):
        ''' Calculates the energy given the scaled atomic properties (in order
            to prevent recomputation if atomic properties are calculated) '''
        if batch_seg is None:
            batch_seg = torch.zeros_like(Z).type(torch.int64)
        ...
        Ea = torch.squeeze(segment_sum(Ea,batch_seg,device=self.device))
        Qa = torch.squeeze(segment_sum(Qa, batch_seg, device=self.device))
        l00 = torch.squeeze(segment_sum(l00,batch_seg,device=self.device))
        l10 = torch.squeeze(segment_sum(l10,batch_seg,device=self.device))
        l11 = torch.squeeze(segment_sum(l11,batch_seg,device=self.device))
        nu = torch.squeeze(segment_sum(nu,batch_seg,device=self.device))

        pred = self.multidimensional_evidential_layer(Ea,Qa,l00,l10,l11,nu)
    ...
    def energy_and_forces_from_atomic_properties(
            self, Ea, Qa, Z, R, idx_i, idx_j, batch_seg=None, create_graph=True):
...
#Here the charges are rescalled and the energy is computed with the new charges
    ...
    def energy_evidential_from_scaled_atomic_properties(
        ...
        return self.energy_evidential_from_atomic_properties(
            Ea, lambdas, alpha, beta, Qa, Dij, Z, idx_i, idx_j, batch_seg)

    @torch.jit.export
    def energy_md_evidential_from_scaled_atomic_properties(
            self, Ea, Qa, l00,l10,l11,nu, Dij, Z, idx_i, idx_j, Q_tot=None, batch_seg=None):
        ''' Calculates the energy given the atomic properties (in order to
            prevent recomputation if atomic properties are calculated) '''
        ...
        pred = self.energy_md_evidential_from_atomic_properties(
            Ea,Qa,l00,l10,l11,nu, Dij, Z, idx_i, idx_j, batch_seg)

        # Return individual charges to calculate dipole moment
        return pred,Qa
    ...
    def energy_from_scaled_atomic_properties(
        ...
        return self.energy_from_atomic_properties(
            Ea, Qa, Dij, Z, idx_i, idx_j, batch_seg)

    @torch.jit.export
    def energy_and_forces_from_scaled_atomic_properties(
            self, Ea, Qa, Z, R, idx_i, idx_j, Q_tot=None, batch_seg=None, create_graph=True):
...
#Obtention of global properties
    ...
    def energy_evidential(self, Z, R, idx_i, idx_j, Q_tot=None,batch_seg=None, offsets=None,
        ...
        energy, lambdas, alpha, beta = self.energy_evidential_from_scaled_atomic_properties(
            Ea, lambdas, alpha, beta, Qa, Dij, Z, idx_i, idx_j, Q_tot, batch_seg)

        return energy, lambdas, alpha, beta

    @torch.jit.export
    def energy_md_evidential(self, Z, R, idx_i, idx_j, Q_tot=None,batch_seg=None, offsets=None,
            sr_idx_i=None, sr_idx_j=None, sr_offsets=None):
        ''' Calculates the total energy (including electrostatic
            interactions) '''
        Ea, Qa, l00, l10, l11, nu, Dij_lr, _ = self.multidimensional_evidential_atomic_properties(
            Z, R, idx_i, idx_j, offsets, sr_idx_i, sr_idx_j, sr_offsets)

        pred, _ = self.energy_md_evidential_from_scaled_atomic_properties(Ea, Qa, l00,l10,l11,nu,
                                                                       Dij_lr, Z, idx_i, idx_j, Q_tot, batch_seg)
        return pred
    ...
    @torch.jit.export
    def energy_and_forces_evidential(self, Z, R, idx_i, idx_j, Q_tot=None,batch_seg=None, offsets=None,
            sr_idx_i=None, sr_idx_j=None, sr_offsets=None, create_graph=True):
        ''' Calculates the total energy (including electrostatic
            interactions)
            NOTE: This function computes the forces as the derivative of the energy
        ...
        return energy, lambdas, alpha, beta, forces

    @torch.jit.export
    def energy_and_forces_md_evidencial(self,Z,R,idx_i,idx_j,Q_tot=None,batch_seg=None, offsets=None,
                                        sr_idx_i=None, sr_idx_j=None, sr_offsets=None, create_graph=True):
        '''
        Calculate the energy,forces and  dipole given the atomic properties
        '''
        ...
        pred, Qas = self.energy_md_evidential_from_scaled_atomic_properties(Ea, Qa, l00,l10,l11,nu,
                                                                       Dij_lr, Z, idx_i, idx_j, Q_tot, batch_seg)
        if len(pred.size()) != 1:
            energy = torch.sum(pred[:,0])
        else:
            energy = torch.sum(pred[0])

        if idx_i.numel() > 0:  # autograd will fail if there are no distances
            grad = torch.autograd.grad([energy], [R], create_graph=create_graph)[0]
            if grad is not None:  # necessary for torch.jit compatibility
                forces = -grad
            else:
                forces = torch.zeros_like(R)
        ...
        # Calculate dipole
        QR_t = torch.stack([Qas*R[:,0],Qas*R[:,1],Qas*R[:,2]],dim=1)
        dip = segment_sum(QR_t,batch_seg,device=self.device)

        return pred, dip, forces

    @torch.jit.export
    def energy_forces_and_others_evidential(self, Z, R, idx_i, idx_j, Q_tot=None,batch_seg=None, offsets=None,
            sr_idx_i=None, sr_idx_j=None, sr_offsets=None, create_graph=True):
        ''' Calculates the total energy (including electrostatic
            interactions)
            NOTE: This function computes the forces as the derivative of the energy
        ...
        return energy, lambdas, alpha, beta, Qa, forces


    @torch.jit.export
    def energy_forces_and_hessian_evidential(
        ...
        # Taken from SpookyNet implementation
        grad = -forces.view(-1)
        s = grad.size(0)
        ...
        return energy, lambdas, alpha, beta, Qa,forces, hessian

    def energy_hessian_and_forces_md_evidencial(self,Z,R,idx_i,idx_j,Q_tot=None,batch_seg=None, offsets=None,
                                        sr_idx_i=None, sr_idx_j=None, sr_offsets=None, create_graph=True):
        '''
        Calculate the energy,forces and  dipole given the atomic properties
        '''
        ...
        return pred, dip, forces,hessian
